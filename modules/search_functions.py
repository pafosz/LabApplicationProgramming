...
def search(date: datetime) -> list | None:
    for day in af.read_data('datasets/dataset.csv'):
        if day[0] == str(date):
            return day[1:]
...
def search_by_year(date: datetime) -> list | None:
    ...
    for filename in os.listdir(dir):
        if (int(filename[0:4]) == date.year):
            data = af.read_data(f"{dir}/{filename}") 
            for day in data:
                if(day[0]==str(date)):
                    return day[1:]
...
def search_by_week(date: datetime) -> list | None:
    ...
    for filename in os.listdir(dir):
        ...
        if(left_date <= date <= right_date):
            for day in af.read_data(f"{dir}/{left_date}_{right_date}"):
                if(day[0] == str(date)):
                    return day[1:]      
                
def search_by_xy(date: datetime) -> list | None:
    data_by_x = af.read_data('datasets/date_and_data/X.csv')
    for row, value in enumerate(data_by_x):
        row_date = "-".join(value)
        if row_date == str(date):
            data_by_y = af.read_data('datasets/date_and_data/Y.csv')
            return data_by_y[row]

def next() -> tuple:
    index = 0
    data = af.read_data(r"C:\Users\Admin\Desktop\University\2 cours\3 semester\PP\LabAppProg\LabApplicationProgramming\datasets\dataset.csv")
    if index < len(data) - 1:
        index += 1
